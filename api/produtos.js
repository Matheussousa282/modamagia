  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const query = `
        SELECT 
          ...
          CASE
            WHEN pr.tipo = 'percentual'
              THEN ROUND(p.preco - (p.preco * (pr.valor / 100)), 2)

            WHEN pr.tipo = 'fixo'
              THEN GREATEST(p.preco - pr.valor, 0)
    ...
    // =========================================================
      ...
      const tamanhosArray = Array.isArray(tamanhos)
        ? tamanhos.map(t => String(t).trim()).filter(Boolean)
        : [];
      ...
      const coresArray = Array.isArray(cores)
        ? cores.map(c => String(c).trim()).filter(Boolean)
        : [];
      ...
      const imagensArray = Array.isArray(imagens)
    ...
    // =========================================================
      ...
      if (!nomeAntigo) {
        ...
        return res.status(400).json({
          message: "Nome antigo não informado."
        });
